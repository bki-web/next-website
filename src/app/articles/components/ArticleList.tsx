...
export default function ArticleList() {
  ...
  const { data, isLoading, isError, error } = trpc.article.getList.useQuery({
    page: currentPage,
    limit: 10,
  });
  ...
  if(data && data.data && data.data.length === 0){
    return (
        <div>
            <EmptyStateCard title="Article not found" message="Please check again another time while we update this page" />
        </div>
    )
  }
