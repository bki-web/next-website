...
export default function SliderSection({
    ...
    useEffect(() => {
        ...
        timerRef.current = setTimeout(
            () => setSlideIndex((i) => (i + 1) % currentSlide.length),
            AUTOPLAY_MS
        );
        return () => {
            if (timerRef.current) clearTimeout(timerRef.current);
        };
    }, [currentSlide.length, slideIndex]);
    ...
    const slide = currentSlide[slideIndex];

    const nextSlide = () => setSlideIndex((i) => (i + 1) % currentSlide.length);
    const prevSlide = () =>
